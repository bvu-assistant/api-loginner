...
async function getTestSchedules(url, ssid)
{
    try
    {
        let stream = await new Promise(async (resolve, reject) =>
        {
            ...
            let body = await getTestScheduleHTML(url, ssid);
            ...
            resolve(response.Term === ''? 'null' : response);
        });
...
async function getTestScheduleHTML(url, ssid)
{
    try
    {
        if (url === undefined)
